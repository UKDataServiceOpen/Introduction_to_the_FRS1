...
In this exercise, we merge data from the Family Resources Survey (FRS), 2022-2023 financial year to explore the association between variables available in different FRS datasets and to assess the effect of survey weights on the results using R.
...
Now, let’s also have a look at the frs_pension2223_short dataset.

```{r}
dim(frs_pension2223_short)    #Gives the number of rows (observations) and columns (variables)
```

```{r}
names(frs_pension2223_short)  #List variable names in their actual order in the dataset
```

```{r}
head(data.frame(frs_pension2223_short))     #Displays the first six lines of a dataset
```
...
Let’s examine the variables’ description and value labels of HEALTH1, SEX and IAGEGR4 from frs_adult2223_short dataset.
...
Now, let’s examine the variables’ description and value labels of PENPAY and PENTYPE from frs_pension2223_short dataset.
...
In that case, we need to merge data as the two variables of interest are in two different datasets: SEXf, which measures gender is in frs_adult2223_short, and PENTYPE, which measures Pension Type is in frs_pension2223_short.
