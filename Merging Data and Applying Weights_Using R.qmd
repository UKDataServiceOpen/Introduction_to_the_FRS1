...
Since the FRS data are relational database collected at different level: households, families (benefit units), and individuals (persons), and the datasets are available in several data files, with some focus on specialised topics such as assets, childcare, mortgage, pensions …etc., you may want at some point to conduct analysis at different data levels or need to explore the association between the FRS variables but they are available in different data files.
